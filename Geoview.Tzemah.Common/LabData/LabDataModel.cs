...
namespace Geoview.Tzemah.Common.LabData
{
    ...
    public class LabDataModel : Expando
    {
        ...
        public LabDataModel(SurveyPits pits, SoilSurveyMaterialsTable soilSurveyMaterials, int surveyId)
        {
            //PitId = int.Parse(soilSurveyMaterials["PitId"].ToString());
            ...
            //DepthId = int.Parse(soilSurveyMaterials["DepthId"].ToString());
            ...
            foreach (KeyValuePair<string, object> material in soilSurveyMaterials)
            {
                try
                {
                    if (material.Key.Contains("Date"))
                    {
                        this[material.Key] = DateTime.ParseExact(material.Value.ToString(), "dd/MM/yyyy", new CultureInfo("en-US", true));
                    }
                    else if (material.Key == "PitId")
                    {
                        this[material.Key] = pits.getPitIdVal(material.Value.ToString());
                    }
                    else
